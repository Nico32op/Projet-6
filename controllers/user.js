...
exports.signup = (req, res, next) => {
  ...
  const regex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/; //au moins 8 caractères, 1 lettre minuscule, 1 majuscule et 1 chiffre

  if (req.body.password.match(regex)) {
    //l'utilisateur ne peut-être créer que si le mot de passe reçu dans la requête correspond à la méthode regex
    bcrypt
